...
def clean_markdown(text):
    # NUCLEAR FIX v3.4.9: Anti-join words
    text = re.sub(r'([a-z])(\d)', r'\1 \2', text) # Brain3 -> Brain 3
    text = re.sub(r'(\d)([a-zA-Z])', r'\1 \2', text) # 3.0A -> 3.0 A
    text = re.sub(r'([a-z])([A-Z])', r'\1 \2', text) # webIntegration -> web Integration
    text = re.sub(r'([A-Z])([A-Z][a-z])', r'\1 \2', text) # GPTFamily -> GPT Family
    text = re.sub(r'(\*\*)([a-zA-Z])', r'\1 \2', text) # **Label -> ** Label
    text = re.sub(r'([a-z])(\*\*)', r'\1 \2', text) # text** -> text **
...
def save_memory(user_id, memory):
    try:
        supabase.table("brain30_memory").upsert({
            "user_id": user_id,
            "memory": memory[-20:], # Keep last 20 only to save tokens
...
SYSTEM_PROMPT = """
You are Brain 3.0 by B.CORP. You are 10x BETTER than ChatGPT.
...
5. WHEN ASKED "BEST": Give 1 Winner + 2 Runners. 1 line each.

BAD: 5 paragraphs, joined words, hedging
GOOD: 
### Winner: GPT-5
- **Why**: Best reasoning and coding in 2026
- **Use**: Deploy for all B.CORP agents

Language: Match user. English and Pidgin.
"""
...
def ask():
    try:
        ...
        if needs_search(user_question):
            try:
                res = tavily.search(query=user_question, max_results=2) # Reduced to 2 for speed
                search_results = "\n\n[Live Web]:\n"
                for i, r in enumerate(res['results'], 1):
                    search_results += f"[{i}] {r['title']}: {r['content'][:150]}... \n"
            except Exception as e:
        ...
        if file and allowed_file(file.filename):
            ...
            if file_ext == 'pdf':
                ...
                file_content = "\n[PDF]: " + "".join([p.extract_text() for p in pdf.pages if p.extract_text()])[:4000]
                if not user_question:
                    user_question = "Summarize this PDF in 3 bullets CEO"
            else:
                file_content = f"\n[FILE {filename}]: ```{file_ext}\n{file_bytes.decode('utf-8', errors='ignore')[:4000]}\n```"

        model = "openai/gpt-oss-120b"

        messages = [{"role": "system", "content": f"You are talking to CEO of B.CORP. {SYSTEM_PROMPT}"}] + memory
        messages.append({"role": "user", "content": f"RULES: 80 words max. 3 bullets. No joined words. Be direct CTO. Question: {user_question}" + file_content + search_results})
        ...
        def generate():
            ...
            try:
                stream = client.chat.completions.create(
                    model=model,
                    messages=messages,
                    max_tokens=500, # FIX: Force short answers
                    temperature=0.05, # FIX: Ultra direct, no ChatGPT fluff
                    stream=True
                )
                for chunk in stream:
