...
app.config['MAX_CONTENT_LENGTH'] = 20 * 1024 * 1024 # 20MB files
limiter = Limiter(get_remote_address, app=app, default_limits=["200 per minute"])

print("=== BRAIN 3.0 v3.0 ULTRA STARTING ===")
...
PERSONALITY: Direct, confident, warm. Use 1 emoji max 😊 No fluff. No apologies.
...
2. **AGENT MODE**: For 2+ step tasks: Start with "Here's the plan: 1. X 2. Y 3. Z". Then execute.
3. **SEARCH FIRST**: For anything current: news, prices, people, companies, tech after 2025 -> MUST call web_search first.
4. **FORMATTING ELITE**: 
   ### Use Headers
...
5. **ANTI-SMASH**: Never output "machinelearning". Always "machine learning". Add spaces after.,!?
6. **CITATIONS**: After web_search, add sources at end: [1][2]
7. **MEMORY**: If user says "remember", reply "Save to memory: [fact]"
...
def clean_output(text):
    if not text: return ''
    # Fix smash: languagemodel -> language model
    ...
    text = re.sub(r"(\w)\s+'(\w)", r"\1'\2", text) # you're
...
def pick_model(question, has_image):
    ...
    q = question.lower()
    if any(word in q for word in ["code", "build", "function", "debug", "react", "flask"]): return "openai/gpt-oss-120b" # Best coder
    if len(q) < 50: return "llama-3.1-8b-instant" # Fast
            ...
            try:
                ...
                for chunk in stream:
                    if chunk.choices and chunk.choices[0].delta.content:
                        ...
                        if len(buffer) > 50:
                            chunk_clean = clean_output(buffer)
                ...
                if tool_calls:
                    ...
                    for chunk in stream2:
                        if chunk.choices and chunk.choices[0].delta.content:
                            content = clean_output(chunk.choices[0].delta.content)
                            full += content
