...
def clean_markdown(text):
    # V3.1.6 FIX: Don't split model names like Brain 3.0
    text = re.sub(r'([a-z])([A-Z])', r'\1 \2', text) # CEOthe -> CEO the
    text = re.sub(r'([a-zA-Z])(\d)', r'\1 \2', text) # v3 -> v 3, but not Brain3.0
    text = re.sub(r'(\d)([a-zA-Z])', r'\1 \2', text) # 3Brain -> 3 Brain
    text = re.sub(r'(\w):(\w)', r'\1: \2', text) # as:unsupervised -> as: unsupervised
    text = re.sub(r'(\w),(\w)', r'\1, \2', text) # CEO,CEO -> CEO, CEO
    text = re.sub(r'(\w)-(\w)', r'\1 - \2', text)
    text = re.sub(r'([^\n])###', r'\n\n###', text) # 2 blank lines before headings
    text = re.sub(r'([^\n])(\d+\.)', r'\n\2', text) # new line before lists
    text = re.sub(r'([^\n])- \*\*', r'\n- **', text) # new line before bullets
...
def generate_image(prompt):
    try:
        ...
        # FIX: Handle both list and FileOutput
