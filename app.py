...
def clean_markdown(text, mode):
    # 1. Fix squished words: add space between lowercase and uppercase
    text = re.sub(r'([a-z])([A-Z])', r'\1 \2', text)
    # 2. Fix bold formatting
    text = re.sub(r'\*\s*\s*([^*]+)\*\s*\*', r'**\1**', text)
    # 3. Add line breaks before headings and bullets
    text = re.sub(r'(### )', r'\n\n\1', text)
    text = re.sub(r'(- \*\*)', r'\n\1', text)
    # 4. Remove code blocks if not in code mode
    if mode!= "code":
...
2. Use ### Headings and - **Bold**: bullets with spaces.
3. Address user as "CEO".
4. Keep it 5-8 sentences.
5. End with "Would you like me to... CEO?" """,
    "pidgin": """You be Brain 3.0 by B.CORP. Oga CEO. Answer like Meta AI in Naija Pidgin. Use ### headings. Add spaces. End with question.""",
            ...
            try:
                ...
                for chunk in stream:
                    if chunk.choices and chunk.choices[0].delta and chunk.choices[0].delta.content:
                        ...
                        full += text
                        yield f"data: {text}\n\n"
                        time.sleep(0.01) # prevents chunks merging
            except Exception as e:
                print(f"Stream Error: {traceback.format_exc()}")
                yield f"data: ❌ Error: {str(e)}\n\n"

            # Clean the full response before saving
            cleaned_full = clean_markdown(full, mode)
            try:
                save_memory(user_id, memory + [{"role": "user", "content": user_question}, {"role": "assistant", "content": cleaned_full}])
            except Exception as e:
