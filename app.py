...
def clean_markdown(text, mode):
    # FIX 1: Add space between lowercase and uppercase: TechnologyThe -> Technology The
    text = re.sub(r'([a-z])([A-Z])', r'\1 \2', text)
    # FIX 2: Add space around dash: Dev-Battery -> Dev - Battery
    text = re.sub(r'(\w)-(\w)', r'\1 - \2', text)
    # FIX 3: Add space after colon: Label:text -> Label: text
    text = re.sub(r':(\w)', r': \1', text)
    # FIX 4: Force line breaks before headers
    text = re.sub(r'([^\n])###', r'\1\n\n###', text)
    # FIX 5: Force line breaks before numbered lists
    text = re.sub(r'([^\n])(\d+\.)', r'\1\n\2', text)
    # FIX 6: Force line breaks before bullet points
    text = re.sub(r'([^\n])- \*\*', r'\1\n- **', text)
    # FIX 7: Remove code blocks if not in code mode
    if mode!= "code":
        text = re.sub(r'```[\s\S]*?```', '', text)
    # FIX 8: Fix multiple spaces
    ...
    try:
        ...
        if file and allowed_file(file.filename):
            ...
            if file_ext in ['png', 'jpg', 'jpeg']:
                ...
                file_content = "\n[PDF]: " + "".join([p.extract_text() for p in pdf.pages if p.extract_text()])[:8000]
            else:
        ...
        def generate():
            ...
            try:
                ...
                buffer = "" # FIX 10: Buffer to prevent chunk breaking words
                for chunk in stream:
                    if chunk.choices and chunk.choices[0].delta and chunk.choices[0].delta.content:
                        buffer += chunk.choices[0].delta.content
                        # Only yield when we hit space or punctuation
                        if re.search(r'[ \n.,;:!?]$', buffer):
