...
print("=== BRAIN 3.0 v4.5.8 PRODUCTION ===")
...
def extract_docx_text(file_bytes):
    try:
        ...
        doc = Document(io.BytesBytes(file_bytes))
...
# =========================
# ROUTES - THIS FIXES THE 404
# =========================
@app.route("/")
def home():
    # If you have a PWA in /templates/index.html it will load
    # Otherwise it returns API status so you know it's alive
    try:
        return render_template("index.html")
    except:
        return jsonify({
            "status": "B.CORP Brain 3.0 v4.5.8 is LIVE",
            "version": "4.5.8",
            "docs": "POST to /api/chat with Authorization: Bearer <supabase_token>"
        })

@app.route("/health")
def health():
    return jsonify({"ok": True, "service": "brain-3.0"})
