...
from brain_core import ModelRouter,get_chat_history,get_last_user_message,save_to_rag,should_use_agent
...
def create_app():
    ...
    app.config["MODEL_ROUTER"]=router
    ...
    def auth_config():
        return jsonify({"supabase_url":settings.supabase_url,"supabase_anon_key":settings.supabase_anon_key,"auth_enabled":True,"version":VERSION})
    ...
    def health():
        ...
        return jsonify({"status":"ok" if ok else "degraded","service":settings.app_name,"version":VERSION,"environment":settings.environment,"database":"connected" if ok else "disconnected","auth":"enabled"})

    @app.get("/api/chats")
    @auth_required
    def chats():
        return jsonify(list_chats(current_user_id(),request.args.get("limit",50,type=int)))
    ...
    def delete_chat_route():
        ...
        if not chat_id:
            return jsonify({"error":"chat_id required","code":"VALIDATION_ERROR"}),400
    ...
    def chat():
        ...
        if requested_chat_id:
            ...
            new_chat=create_chat(uid,question[:60] if question else "New Chat")
            chat_id=str(new_chat["id"])

        if not is_regen:
            try:
                existing=db().table("messages").select("id").eq("client_msg_id",client_msg_id).eq("user_id",uid).limit(1).execute()
                if existing.data:
                    return jsonify({"error":"Duplicate message.","code":"DUPLICATE_MESSAGE"}),409
            except Exception:
                logger.exception("Idempotency check failed")
                return jsonify({"error":"Database error.","code":"DATABASE_ERROR"}),500
        ...
        if uploaded_file and uploaded_file.filename:
            ...
            try:
                ...
                db_file_meta={"name":original_name,"type":ext,"size":size}
                if not is_image_file(original_name) and (settings.openai_api_key or settings.jina_api_key):
                    rag_stats=save_to_rag(chat_id,uid,filepath,db())
            except Exception:
        ...
        messages=get_chat_history(chat_id,uid,db())
        last_user_msg=get_last_user_message(messages)
        use_agent=mode=="agent" or should_use_agent(last_user_msg) or bool(file_meta and not is_image_file(file_meta["name"]))
