...
# NEW: Set Replicate token
os.environ["REPLICATE_API_TOKEN"] = config.REPLICATE_API_TOKEN # Add this to config.py
...
def needs_image(query): # NEW
    keywords = ['imagine', 'generate image', 'create image', 'draw', 'picture of']
    return any(word in query.lower() for word in keywords)

def generate_image(prompt): # NEW
    try:
        output = replicate.run(
            "black-forest-labs/flux-schnell",
            input={
                "prompt": f"B.CORP style, high quality, {prompt}",
                "aspect_ratio": "1:1",
                "output_format": "webp",
                "num_outputs": 1
            }
...
You can see images, read PDFs, write code, browse the web, AND generate images with /imagine.
...
def ask():
    try:
        ...
        # NEW: Check if user wants image generation
        if needs_image(user_question):
            prompt = user_question.replace("imagine", "").replace("generate image", "").replace("create image", "").strip()
            image_url = generate_image(prompt)
            if image_url:
                return jsonify({
                    "answer": f"### Generated Image CEO ###\n\nHere is what I created: {prompt}\n\n![Generated Image]({image_url})",
                    "image": image_url
                })
            else:
