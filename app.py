...
def create_chat(user_id, first_message):
    ...
    try:
        ...
        if res.data:
            ...
            print("CREATE_CHAT ERROR: No data returned", res)
            return None
    except Exception as e:
...
def save_message(chat_id, role, content):
    ...
    try:
        ...
        try:
            ...
            print("UPDATE CHAT TIME ERROR:", e)
    except Exception as e:
...
def get_chats():
    user_id = request.args.get("user_id", "guest") # DEFAULT TO GUEST
...
def ask():
    try:
        ...
        user_id = request.form.get("user_id", "guest") # DEFAULT TO GUEST - FIX 401
        chat_id = request.form.get("chat_id")
        file = request.files.get("file")

        is_new_chat = False # TRACK IF NEW
        if not chat_id or chat_id == "null":
            chat_id = create_chat(user_id, user_question or "New Chat")
            is_new_chat = True # FLAG
            if not chat_id:
                return jsonify({"error": "Failed to create chat. Check Supabase permissions."}), 500

        messages_db = load_messages(chat_id)
        memory = [{"role": m['role'], "content": m['content']} for m in messages_db[-10:]]

        # AUTO SEND INTRODUCTION ON FIRST MESSAGE
        if is_new_chat and len(messages_db) == 0:
...
Ask me anything to get started 😊"""
            save_message(chat_id, "assistant", intro_text)
        ...
        if user_question: # ONLY SAVE IF NOT EMPTY
