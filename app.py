...
def clean_markdown(text):
    if not text:
        return ''
    # Safe version: only fix camelCase and missing spaces. Keeps ### ** - intact
    text = re.sub(r'([a-z])([A-Z])', r'\1 \2', text) # wordWord -> word Word
    text = re.sub(r'([a-z])(\d)', r'\1 \2', text) # word1 -> word 1
    text = re.sub(r'(\d)([a-zA-Z])', r'\1 \2', text) # 1word -> 1 word
    text = re.sub(r'([a-z])\.([A-Z])', r'\1. \2', text) # sent.Sent -> sent. Sent
    text = re.sub(r'([a-z]):([A-Z])', r'\1: \2', text) # label:Label -> label: Label
