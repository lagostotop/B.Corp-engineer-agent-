...
logger.info("Brain 3.0 started | version=%s | auth=disabled | test_user=%s",VERSION,TEST_USER_ID)
...
# ===== DB HELPERS =====
def get_chat_count(uid):
    try: return supabase.table("chats").select("id",count="exact").eq("user_id",uid).execute().count or 0
    except: return 0
def get_msg_count(uid):
    try: return supabase.table("messages").select("id",count="exact").eq("user_id",uid).execute().count or 0
    except: return 0
def get_doc_count(uid):
    try: return supabase.table("documents").select("id",count="exact").eq("user_id",uid).execute().count or 0
    except: return 0
def cleanup_old(uid,days=30):
    cutoff=(datetime.now(timezone.utc)-timedelta(days=days)).isoformat()
    try:
        old=supabase.table("chats").select("id").eq("user_id",uid).lt("updated_at",cutoff).execute()
        deleted=0
        for c in old.data or []:
            supabase.table("chats").delete().eq("id",c["id"]).execute()
            deleted+=1
        cache_del_pattern(f"chat:{uid}:*")
        cache_del_pattern(f"chats:{uid}")
...
def health():
    ...
    return jsonify({"status":"ok" if db else "degraded","service":"brain3","version":VERSION,
        "auth":"disabled","database":"connected" if db else "disconnected",
...
def rh():
    ...
    return jsonify({"status":"healthy" if(db and(not redis_client or rs))else"degraded",
        "redis":rs,"database":db,"supabase":bool(supabase),
        "groq":bool(os.getenv("GROQ_API_KEY")),"version":VERSION,
        "uptime":time.time()-app.start_time if hasattr(app,'start_time')else 0})

@app.route("/metrics")
def metrics():
    return jsonify({"version":VERSION,"redis_connected":bool(redis_client),
        "supabase_connected":bool(supabase),
        "environment":os.getenv("FLASK_ENV","development"),
        "start_time":datetime.fromtimestamp(app.start_time).isoformat() if hasattr(app,'start_time')else None})
...
def db_stats():
    uid=get_uid()
    try:
        return jsonify({"total_chats":get_chat_count(uid),"total_messages":get_msg_count(uid),
            "total_documents":get_doc_count(uid),"user_id":uid,
            "timestamp":datetime.now(timezone.utc).isoformat()})
    except Exception as e:
...
def cleanup_db():
    ...
    try: return jsonify(cleanup_old(uid,data.get("days",30)))
    except Exception as e: return jsonify({"error":"Cleanup failed"}),500
