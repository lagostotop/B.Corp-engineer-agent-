...
app = Flask(__name__, static_folder="static", template_folder="templates") # lowercase
...
IRON RULES:
1. NO JOINED WORDS: "machine learning" NOT "machinelearning"
...
def ask():
    try:
        ...
        messages.append({"role": "user", "content": f"Answer in Meta AI style. Use headers, bullets, and clear spacing. Max 150 words. Question: {user_question}" + file_content + search_results})
        ...
        def generate():
            ...
            buffer = ""
            try:
                stream = client.chat.completions.create(model=model, messages=messages, max_tokens=400, temperature=0.7, stream=True) # 400 tokens for Meta AI style
                for chunk in stream:
