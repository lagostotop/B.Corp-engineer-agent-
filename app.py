...
app = Flask(__name__, static_folder="static", template_folder="templates") # note: lowercase "templates"
...
2. SPACING: Always put space after., : 
3. FORMATTING: Use line breaks between sections
...
def clean_markdown(text):
    if not text: return ''
    # FIX JOINED WORDS
    text = re.sub(r'([a-z])([A-Z])', r'\1 \2', text) # Energycarry -> Energy carry
...
# NEW: Serve JS from Templates folder so app.js loads
@app.route('/Templates/<path:filename>')
def serve_templates(filename): return send_from_directory('Templates', filename)
...
def ask():
    try:
        ...
        messages.append({"role": "user", "content": f"Answer professionally. Use proper spacing. Max 120 words. No joined words. Question: {user_question}" + file_content + search_results})
        ...
        def generate():
            ...
            buffer = ""
            try:
                stream = client.chat.completions.create(model=model, messages=messages, max_tokens=300, temperature=0.3, stream=True) # 300 = ~120 words
                for chunk in stream:
