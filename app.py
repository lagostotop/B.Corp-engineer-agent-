...
print("=== BRAIN 3.0 v1.3 ELITE STARTING ===")
...
def ask():
    try:
        ...
        def generate():
            full = ""; buffer = ""
            try:
                # Send chat_id immediately so frontend can update sidebar
                ...
                for chunk in stream:
                    if chunk.choices and chunk.choices[0].delta.content:
                        ...
                        # Stream every 40 chars for smooth typing
                        if len(buffer) > 40:
                ...
                # Flush remaining buffer - FIXED INDENTATION
                if buffer:
                    chunk_clean = clean_markdown(buffer)
                    if chunk_clean.strip():
                        full += chunk_clean
