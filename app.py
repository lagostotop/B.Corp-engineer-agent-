...
print("=== BRAIN 3.0 v5.4.4 PRODUCTION ===")
...
SYSTEM_PROMPT="You are Brain 3.0. Cite [SOURCE 1]. End with ### Confidence ### Sources"
...
def needs_tools(q,has_files):
    q=q.lower()
    return has_files or any(t in q for t in ["latest","news","research","current","today"])

def generate_plan(task):
    if not task:return[]
    try:
        res=client.chat.completions.create(model=GROQ_GENERAL_MODEL,messages=[{"role":"user","content":f'JSON {{"steps":[{{"action":"search"|"recall"|"doc_search","query":"string"}}]}} Task:{task}'}],response_format={"type":"json_object"},max_tokens=400,timeout=10)
...
def call_tool(tool_name,args,user_id,chat_id):
    ...
    if tool_name=="recall_memories":
        emb=get_embedder().encode(q).tolist()
...
def process_uploaded_file(file,user_id,chat_id):
    ...
    if ext=="pdf":
        ...
        for p in pdf.pages[:50]:text+=p.extract_text()or"";
        if len(text)>MAX_EXTRACTED_TEXT or time.time()-start>10:break
    elif ext=="docx":
        from docx import Document
        for p in Document(io.BytesIO(b)).paragraphs:text+=p.text+"\n";
        if len(text)>MAX_EXTRACTED_TEXT or time.time()-start>10:break
    ...
    def worker():
        try:
            embs=get_embedder().encode(chunks,batch_size=16).tolist()
        ...
        except:supabase.table("documents").update({"status":"failed"}).eq("id",doc_id).execute()
    threading.Thread(target=worker,daemon=True).start()
    return f"\n\n[FILE:{fn} {len(chunks)}chunks]",False,None,{"type":"file","name":fn,"doc_id":doc_id}

@app.route("/api/chats",methods=["GET"])
@limiter.limit("30 per minute")
def get_chats():uid=get_user_id();res=supabase.table("chats").select("*").eq("user_id",uid).order("created_at",desc=True).limit(50).execute();return jsonify(res.data or [])

@app.route("/api/chat/<chat_id>",methods=["GET"])
@limiter.limit("30 per minute")
def get_chat(chat_id):return jsonify(load_messages(chat_id,get_user_id()))

@app.route("/api/chat/delete",methods=["POST"])
@limiter.limit("10 per minute")
def delete_chat():uid=get_user_id();cid=(request.get_json()or{}).get("chat_id");
if not cid:abort(400);get_owned_chat(cid,uid);supabase.table("messages").delete().eq("chat_id",cid).eq("user_id",uid).execute();supabase.table("chats").delete().eq("id",cid).eq("user_id",uid).execute();return jsonify({"success":True})
...
def ask():
    ...
    def stream():
        ...
        try:
            ...
            msgs=[{"role":"system","content":SYSTEM_PROMPT}]+[{"role":m["role"],"content":m["content"]}for m in hist]
            um={"role":"user","content":q}
            if has_img:um["content"]=[{"type":"text","text":q},{"type":"image_url","image_url":{"url":img_url}}]
            msgs.append(um)
            for chunk in client.chat.completions.create(model=model,messages=msgs,stream=True,timeout=GENERATION_TIMEOUT):
        ...
        except Exception:e:log.exception("Stream");yield f"event: error\ndata: {json.dumps({'message':'Server error'})}\n\n"
        finally:
