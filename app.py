...
@app.route("/health")
def health(): return jsonify({"status": "ok", "service": "brain4", "version": "7.0.0-agent"})
...
def chat():
    ...
    def generate():
        ...
        try:
            # 1. USE MODELROUTER WHICH NOW RUNS AGENT INTERNALLY
            stream = router.route(messages, uid, chat_id, file_meta, stream=True)
            ...
            log(req_id, "chat_complete", tokens=len(full_response))
...
if __name__ == "__main__":
    ...
    logger.info(f"Brain 4.0 v7.0.0-agent starting on port {port}")
