...
def load_memory(user_id):
    # FIX v3.4.5: Handle empty table + no row
    try:
...
1. NEVER join words. Write "Brain 3.0" NOT "Brain3.0". Write "Overview" NOT "Overvlew"
2. ALWAYS put space after comma: "CRM, ERP, BI" NOT "CRM,ERP,BI"
3. ALWAYS put space between number and word: "14 times" NOT "14times"
4. ALWAYS use markdown: \n\n### Header \n- **Label**: Text
5. If you cut off a word you are broken.

Examples of BAD output: Braln3.0, Overvlew-, CRM,ERP, than14
Examples of GOOD output: Brain 3.0, Overview, CRM, ERP, than 14
...
def ask():
    try:
        ...
        messages.append({"role": "user", "content": f"CRITICAL: Use full words with spaces. No cutoff. Format with ### headers and - **bold bullets**. Question: {user_question}" + file_content + search_results})
        ...
        def generate():
            ...
            try:
                stream = client.chat.completions.create(
                    ...
                    messages=messages,
                    max_tokens=4000,
                    temperature=0.2, # Lowered = less creativity = less cutoff
                    stream=True
                )
                for chunk in stream:
                    if chunk.choices and chunk.choices[0].delta.content:
                        ...
                        # NUCLEAR FIX v3.4.5: Force split joined words immediately
                        ...
                        # FIX: Send every 20 chars instead of 4 for smoother stream
                        if len(buffer) > 20:
                ...
                # Save to memory after stream is done
                try:
