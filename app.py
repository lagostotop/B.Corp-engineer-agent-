...
# FIX: Handle 413 properly
@app.errorhandler(RequestEntityTooLarge)
def handle_413(e):
    return jsonify({"error": "File too large. Max 10MB", "request_id": getattr(g, "request_id", "unknown")}), 413
...
def verify_token(token):
    ...
    try:
        ...
        if user and user.user:
            logger.info("Token verified for user: %s", user.user.id) # DEBUG LOG
            return user.user.id
    except Exception:
        logger.exception("Token verification failed") # This will show if key is wrong
    return None
...
def debug_auth():
    ...
    return jsonify({"has_token": bool(token), "uid": uid, "key_type": "service_role" if os.environ["SUPABASE_KEY"].startswith("eyJ") else "unknown"})
