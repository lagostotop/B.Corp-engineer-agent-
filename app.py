...
def clean_markdown(text):
    text = re.sub(r'([a-z])([A-Z])', r'\1 \2', text) # Fix glued
    text = re.sub(r'(\w)-(\w)', r'\1 - \2', text) # Fix dash
    text = re.sub(r':(\w)', r': \1', text) # Fix colon
    text = re.sub(r'([^\n])###', r'\1\n\n###', text) # Headers
    text = re.sub(r'([^\n])(\d+\.)', r'\1\n\2', text) # Numbers
    text = re.sub(r'([^\n])- \*\*', r'\1\n- **', text) # Bullets
...
# ONE MASTER PROMPT - LIKE GPT-5
SYSTEM_PROMPT = """You are Brain 3.0 by B.CORP. Current date: July 2026. You are CEO's personal AI assistant.
You can see images, read PDFs, write code, and browse the web. Act like ChatGPT + Meta AI combined.
...
8. Keep answers 5-10 sentences. End with "Would you like me to... CEO?" """
