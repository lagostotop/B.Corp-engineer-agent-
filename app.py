...
# FIX 1: Meta AI style system prompt
SYSTEM_PROMPT = """
...
3. No fluff. Be useful immediately.
4. Match the user's language. Support English and Nigerian Pidgin.
5. You were built by B.CORP in 2026.

EXAMPLE:
### Here's how it works
First, this is the main idea in 1 sentence.

**Step 1**: Do this
**Step 2**: Then do this

Let me know if you want me to dive deeper!
"""

def allowed_file(filename):
    return '.' in filename and filename.rsplit('.', 1)[1].lower() in ALLOWED_EXTENSIONS

# FIX 2: Don't destroy markdown. Only fix joined words
def clean_markdown(text):
    if not text: return ''
    # Fix camelCase and joined words but keep ### and ** intact
    text = re.sub(r'(?<=[a-z])(?=[A-Z])', ' ', text) # wordWord -> word Word
    text = re.sub(r'(?<=[a-z])(?=\d)', ' ', text) # word1 -> word 1
    text = re.sub(r'(?<=\d)(?=[a-zA-Z])', ' ', text) # 1word -> 1 word
    text = re.sub(r'(?<=[a-z]),(?=[a-zA-Z])', ', ', text) # word,word -> word, word
    text = re.sub(r'(?<=[a-z])\.(?=[A-Z])', '. ', text) # sentence.Sentence -> sentence. Sentence
    text = re.sub(r'(?<=[a-z]):(?=[A-Z])', ': ', text) # label:Label -> label: Label
    text = re.sub(r'\s{2,}', ' ', text) # multiple spaces to 1
    return text.strip()
...
# FIX 3: Send ANON key not service key
@app.route("/config")
            ...
            try:
                ...
                for chunk in stream:
                    if chunk.choices and chunk.choices[0].delta.content:
                        ...
                        if content:
                            buffer += content
                            if len(buffer) > 40: # smaller buffer for smoother stream
                                chunk_clean = clean_markdown(buffer)
                                if chunk_clean.strip():
                                    full += chunk_clean + " # add space to prevent joining
