...
# FIX: Preserve newlines and markdown, only fix joined words
def clean_markdown(text):
    if not text: return ''
    # Don't touch ### ** - * 1.
    # Only fix: wordWord, word1, 1word, word,word
    ...
    text = re.sub(r'(?<=[a-z])\.(?=[A-Z])', '. ', text)
    text = re.sub(r'(?<=[a-z]):(?=[A-Z])', ': ', text)
    text = re.sub(r'[ \t]{2,}', ' ', text) # only collapse spaces, not newlines
    return text
                        ...
                        if content:
                            ...
                            if len(buffer) > 40:
                                ...
                                if chunk_clean.strip():
                                    full += chunk_clean + " # FIXED: closed the quote
