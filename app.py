...
print("=== BRAIN 3.0 v1.2 ELITE STARTING ===")
...
def get_user_id(req):
    ...
    try:
        ...
        if str(parsed) == "00000-0000-0000-0000-000": return str(uuid.uuid4())
        return str(parsed)
            ...
            try:
                for i in range(3):
                    ...
                    if tool_calls:
                        ...
                        for tc in tool_calls: result = call_tool(tc.function.name, json.loads(tc.function.arguments)); messages.append({"role": "tool", "tool_call_id": tc.id, "content": result})
                        buffer = ""
                    else: break
                if buffer: chunk_clean = clean_markdown(buffer);
                if chunk_clean.strip(): full += chunk_clean; yield f"data: {chunk_clean}\n\n"
                ...
                yield f"data: [DONE]\n\n"
            except Exception as e: print("STREAM ERROR:", traceback.format_exc()); yield f"data: **Brain 3.0 Error:** {str(e)}\n\n"
        return Response(stream_with_context(generate()), mimetype='text/event-stream')
    except Exception as e: print("ASK ERROR:", traceback.format_exc()); return jsonify({"error": str(e)}), 500
