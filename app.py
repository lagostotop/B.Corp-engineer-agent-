...
print("=== BRAIN 3.0 STARTING ===")
print("SUPABASE_URL:", config.SUPABASE_URL[:30] + "..." if config.SUPABASE_URL else "MISSING")
print("SUPABASE_KEY exists:", bool(getattr(config, 'SUPABASE_KEY', None)))
print("SUPABASE_ANON_KEY exists:", bool(getattr(config, 'SUPABASE_ANON_KEY', None)))
...
PERSONALITY: Warm, smart, direct. Use emojis sparingly 😊
FORMATTING: ### Headers, **Bold**, bullets, blank lines
RULE: No joined words. "machine learning" not "machinelearning"
"""
...
@app.route("/config")
def get_config():
    return jsonify({"supabase_url": config.SUPABASE_URL, "supabase_key": config.SUPABASE_ANON_KEY})
...
def ask():
    try:
        ...
        messages.append({"role": "user", "content": f"Answer in Meta AI style. Question: {user_question}" + file_content + search_results})
                        ...
                        if content:
                            ...
                            if len(buffer) > 40:
                                ...
                                if chunk_clean.strip():
                                    full += chunk_clean + " # FIXED: quote is closed now
