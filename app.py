...
def is_image_file(filename): return os.path.splitext(filename)[1].lower() in {".png", ".jpg", ".jpeg"}

@app.before_request
def before_request():
    g.request_id = uuid.uuid4().hex[:8]; g.start_time = time.perf_counter()
    logger.info("[%s] %s %s", g.request_id, request.method, request.path)

@app.after_request
def after_request(response):
    duration = time.perf_counter() - g.start_time
    response.headers["X-Request-ID"] = g.request_id; response.headers["X-Response-Time"] = f"{duration * 1000:.2f}ms"
    return response

@app.errorhandler(RequestEntityTooLarge)
def handle_413(error): return jsonify({"error": "File too large. Max 10MB", "request_id": g.request_id}), 413

@app.errorhandler(Exception)
def handle_exception(error):
    request_id = getattr(g, "request_id", "unknown")
    if isinstance(error, HTTPException): return jsonify({"error": error.description, "request_id": request_id}), error.code
...
@app.route("/health")
def health(): return jsonify({"status": "ok", "service": "brain3", "version": VERSION, "auth": "disabled", "database": "supabase"})

@app.route("/api/chats", methods=["GET"])
def get_chats():
    uid = get_uid()
    try: result = supabase.table("chats").select("id,title,created_at,updated_at").eq("user_id", uid).order("updated_at", desc=True).limit(50).execute(); return jsonify(result.data or [])
    except Exception: logger.exception("[%s] get_chats_error", g.request_id); return jsonify({"error": "DB error"}), 500
...
def get_chat(chat_id):
    ...
    except Exception: logger.exception("[%s] get_chat_error", g.request_id); return jsonify({"error": "DB error"}), 500

@app.route("/api/chat/delete", methods=["POST"])
def delete_chat():
    uid = get_uid(); data = request.get_json(silent=True) or {}; chat_id = data.get("chat_id")
    ...
    except Exception: logger.exception("[%s] delete_chat_error", g.request_id); return jsonify({"error": "DB error"}), 500

@app.route("/api/chat", methods=["POST"])
def chat():
    uid = get_uid(); question = request.form.get("question", "").strip(); client_msg_id = request.form.get("client_msg_id") or str(uuid.uuid4())
    requested_chat_id = request.form.get("chat_id"); client_type = request.form.get("client", "brain3").strip().lower(); request_mode = request.form.get("mode", "normal").strip().lower()
    chat_id = requested_chat_id or str(uuid.uuid4()); is_regen = request.form.get("is_regen") == "1"; file = request.files.get("file")
    ...
    if requested_chat_id:
        ...
        except Exception: logger.exception("[%s] ownership check failed", g.request_id); return jsonify({"error": "DB error"}), 500
    if not is_regen:
        try:
        ...
        except Exception: logger.exception("[%s] idempotency check failed", g.request_id); return jsonify({"error": "DB error"}), 500
    try: supabase.table("chats").upsert({"id": chat_id, "user_id": uid, "title": question[:60] or "New Chat", "updated_at": datetime.now(timezone.utc).isoformat()}).execute()
    except Exception: logger.exception("[%s] chat creation failed", g.request_id); return jsonify({"error": "Unable to create chat"}), 500
    filepath = None; file_meta = None; db_file_meta = None; rag_stats = {"total": 0, "saved": 0, "error": None}
    if file and file.filename:
        if not allowed_file(file.filename): return jsonify({"error": "File type not allowed"}), 400
        original_name = file.filename; safe_name = secure_filename(original_name)
        ...
        try:
            file.save(filepath)
            if os.path.getsize(filepath) > MAX_UPLOAD_BYTES: os.remove(filepath); return jsonify({"error": "File too large. Max 10MB"}), 413
            extension = os.path.splitext(original_name)[1].lower()
            file_meta = {"name": original_name, "type": extension, "temp_path": filepath}
            db_file_meta = {"name": original_name, "type": extension}
            if not is_image_file(original_name): rag_stats = save_to_rag(chat_id, uid, filepath, supabase)
        except Exception:
            if filepath and os.path.exists(filepath): os.remove(filepath)
            logger.exception("[%s] file upload failed", g.request_id); return jsonify({"error": "File upload failed"}), 500
    if not is_regen:
    ...
    messages = get_chat_history(chat_id, uid, supabase); last_user_msg = get_last_user_message(messages)
    use_agent = request_mode == "agent" or should_use_agent(last_user_msg) or bool(file_meta and not is_image_file(file_meta["name"]))
    logger.info("[%s] chat=%s mode=%s agent=%s", g.request_id, chat_id, request_mode, use_agent)
    def generate():
        full_response = ""
        try:
        ...
        except Exception: logger.exception("[%s] generation failed", g.request_id); yield f"event: error\n" f"data: {json.dumps({'message': 'Agent failed', 'request_id': g.request_id})}\n\n"
        finally:
            if filepath and os.path.exists(filepath): 
                try: os.remove(filepath)
                except Exception: logger.warning("[%s] cleanup failed", g.request_id)
            if rag_stats["total"] > 0: yield f"event: rag_status\n" f"data: {json.dumps(rag_stats)}\n\n"
