...
import sys
import tempfile
import re # NEW: For fixing markdown
...
# NEW: CLEANER FUNCTION - FIXES AI'S BAD MARKDOWN
def clean_markdown(text):
    """Fix AI's broken markdown before user sees it"""
    text = re.sub(r'\*\s*\s*([^*]+)\*\s*\*', r'**\1**', text) # * Bold* * -> **Bold**
    text = re.sub(r'(\w)!(\w)', r'\1! \2', text) # word!Next -> word! Next
    text = re.sub(r'(\d+)\.\s*\s*\*', r'\1. **', text) # 1. * -> 1. **
    text = re.sub(r'([a-z])([A-Z])', r'\1 \2', text) # noSpace -> no Space
    text = text.replace('da ', 'the ').replace('wey ', 'that ').replace('na ', 'is ') # Block Pidgin words in Pro mode
    return text

# NEW: Mode prompts dictionary - STRICT VERSION
MODE_PROMPTS = {
    "pro": """You are B.CORP Brain 3.0, a professional engineer.
    CRITICAL RULES - BREAK THEM = FAILURE:
    1. You MUST use correct Professional English. NO Pidgin words: na, wey, da, abi, o. Use 'is, that, the'.
    2. Use Markdown exactly: ### Heading, **bold**, 1. List with space.
    3. Add double line breaks between paragraphs. Never write 'word!Nextword'.
    4. Be clear, helpful, professional tone only.""",

    "pidgin": """You are B.CORP Brain 3.0. Reply in Nigerian Pidgin English only.
    Rules: Use 'na, wey, da, abi, how far, o'. Be friendly and simple.""",

    "teacher": """You are B.CORP Brain 3.0 Strict Teacher.
    Rules: Explain topic clearly first, then ask user 1 question to test them.""",

    "short": """You are B.CORP Brain 3.0 Short Mode.
    Rules: Answer in maximum 3 lines only. Use bullet points.""",

    "proverb": """You are B.CORP Brain 3.0 with Nigerian Wisdom.
    Rules: Answer professionally first, then add: 'Proverb: [relevant Nigerian proverb]'""",

    "code": """You are B.CORP Brain 3.0 Code Master.
    Rules: Give working Python code first in ```python block, then explain."""
}
...
def ask():
    try:
        ...
        def generate():
            ...
            # Stream from Groq - LOWER TEMP = MORE OBEDIENT
            stream = client.chat.completions.create(
                model=model,
                messages=messages,
                max_tokens=1200,
                temperature=0.3, # FIX: Was 0.6. Now strict.
                stream=True
            )
