...
@app.route("/health")
def health(): return jsonify({"status": "ok", "service": "brain3", "version": "5.6.6"})
...
def chat():
    ...
    # DON'T SAVE USER MSG AGAIN ON REGEN
    if not is_regen:
    ...
    def generate():
        ...
        try:
            ...
            log(req_id, "stream_error", error=str(e))
            yield f'event: error\ndata: {json.dumps({"message": "Something went wrong", "request_id": req_id})}\n\n'
        finally:
...
if __name__ == "__main__":
    ...
    logger.info(f"Brain 3.0 v5.6.6 starting on port {port}")
