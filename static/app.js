...
let userId = null, isSending = false, selectedFile = null, lastQuestion = "", controller = null, currentChatId = null, introShown = false, authToken = null, DEBUG = false; // PROD: false
...
function getAuthHeaders() { const h = { "Accept": "application/json" }; if (authToken) h.Authorization = `Bearer ${authToken}`; return h } // FIX I

document.addEventListener("DOMContentLoaded", initializeApp);

async function initializeApp() { // FIX 2
    try {
        initEvents(); // CRITICAL: was missing
...
async function loadSidebar() {
    ...
    try {
        ...
        const r = await fetch(`${API_BASE}/api/chats`, { method: "GET", headers: getAuthHeaders(), cache: "no-store" }); // FIX A
        ...
        chats.forEach(c => {
            ...
            item.className = `chat-item${String(c.id) === String(currentChatId)? " active" : ""}`; // FIX F
...
async function loadChat(id) {
    ...
    try {
        log("Loading chat:", id)
        const r = await fetch(`${API_BASE}/api/chat/${encodeURIComponent(id)}`, { method: "GET", headers: getAuthHeaders(), cache: "no-store" }); // FIX A
        if (r.status === 401) { alert("Session expired"); doLogout(); return }
        if (r.status === 403) { alert("Access denied"); return }
        if (!r.ok) throw new Error(`Server ${r.status}`); // FIX J
...
async function deleteChat(id) {
    ...
    try {
        const r = await fetch(`${API_BASE}/api/chat/delete`, { method: "POST", headers: { "Content-Type": "application/json",...getAuthHeaders() }, body: JSON.stringify({ chat_id: id }) }); // FIX A
        if (r.status === 401) { alert("Session expired"); doLogout(); return }
        if (r.status === 403) { alert("Access denied"); return }
        if (!r.ok) throw new Error(`Server ${r.status}`); // FIX J
...
function updateAttachmentButton() { const b = $("uploadBtn"); if (!b) return; if (selectedFile) { b.textContent = "📎1"; b.title = `Attached: ${selectedFile.name}` } else { b.textContent = "📎"; b.title = "Attach file" } } // FIX B
...
function renderAI(text) { // FIX D + E
    try {
        ...
        if (m) {
            const links = [...m[1].matchAll(/\[(\d+)\]\s*\[(.*?)\]\((https?:\/\/[^\s)]+)\)/g)]; // FIX E
            if (links.length) {
                sourcesHTML = '<div class="sources"><b>Sources</b>';
                links.forEach(x => {
                    const url = safeHttpUrl(x[3]);
                    if (url) sourcesHTML += `<div class="source-card"><a href="${url}" target="_blank" rel="noopener noreferrer nofollow">[${escapeHTML(x[1])}] ${escapeHTML(x[2])}</a></div>` // FIX E
                });
    ...
    } catch (e) { console.error(e); return `<div style="white-space:pre-wrap">${escapeHTML(text)}</div>` } // FIX D
}

function getTime() { return new Date().toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" }) }
function addMessage(role, text, ts = null) {
    const box = $("chatBox"); if (!box) return null;
    const wrap = document.createElement("div"); wrap.className = `message-wrap ${role}`; // FIX F
    ...
    if (role === "ai") {
        ...
        const meta = document.createElement("div"); meta.className = "msg-meta"; meta.textContent = `${time} ✓✓`; // FIX G
...
async function sendMessage(q = null, regen = false) { // FIX 12
    let ai = null;
    try {
        if (isSending) return;
        await getAuthToken();
        ...
        if (!regen) { let dt = question; if (file) dt += `\n[Attached: ${file.name}]`; addMessage("user", dt) } // FIX H
        ...
        log("Sending to /api/chat", { has_file:!!file, chat_id: currentChatId })
        const r = await fetch(`${API_BASE}/api/chat`, { method: "POST", headers: { "Authorization": `Bearer ${authToken}`, "Accept": "text/event-stream" }, body: fd, signal: controller.signal, cache: "no-store" }); // FIX I
