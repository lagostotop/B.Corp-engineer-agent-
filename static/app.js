...
// UPGRADE: MORE AGGRESSIVE ANTI-SMASH
function antiSmash(text) {
    if(!text) return '';
    text = text.replace(/web_search\s*\{.*?\}/gs, '');
    text = text.replace(/save_to_memory\s*\{.*?\}/gs, '');
    text = text.replace(/(\w)"(\w)/g, '$1 "$2'); // FIX [1]"TITLE
...
function renderAI(text) {
    try {
        ...
        // UPGRADE: FIX SOURCES REGEX. You had **Sources* with 1 star
