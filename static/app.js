...
async function getAuthToken(){
  const { data: { session } = await supabase.auth.getSession(); // FIX: Check real session
  authToken = session?.access_token || null;
  userId = session?.user?.id || null;
  if(authToken){
    localStorage.setItem("brain30_access_token", authToken);
...
async function initializeApp(){
  ...
  s.onload = async () => {
    ...
    await getAuthToken();
    if(!authToken){ showAuthScreen() }
    else { showChatScreen() }
  }
  s.onerror = () => { alert("Failed to load Supabase SDK"); }
...
async function doLogin(){
  ...
  localStorage.setItem("brain30_access_token", data.session.access_token);
  localStorage.setItem("brain30_user_id", data.user.id);
  showChatScreen(); // FIX: Don't reload, just show app
  loadSidebar();
  showIntroduction();
}
...
async function doSignup(){
  ...
  // FIX 2: If user is returned with session, log them in immediately
  if(data.session){
    localStorage.setItem("brain30_access_token", data.session.access_token);
    localStorage.setItem("brain30_user_id", data.user.id);
    $("authError").style.color = "#22c55e";
    $("authError").textContent = "Account created! Logging you in...";
    setTimeout(()=>{ showChatScreen(); loadSidebar(); showIntroduction(); }, 800);
  } else {
    // Email confirmation required
    $("authError").style.color = "#22c55e";
    $("authError").textContent = "Account created! Check email to confirm, then Sign In.";
  }
}

function doLogout(){
  localStorage.removeItem("brain30_access_token");
  localStorage.removeItem("brain30_user_id");
