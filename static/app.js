...
function debug(...args){ if(DEBUG) console.log("%c[Brain3 App]","color:#00BFFF;font-weight:bold",...args) }
...
function antiSmash(t) { if (!t) return ""; return String(t).replace(/web_search\s*\{[\s\S]*?\}/gi, "").replace(/save_to_memory\s*\{[\s\S]*?\}/gi, "").trim() }
...
function renderAI(text) {
    try {
        ...
        // FIX: Regex was \*\*Sources\*\ should be \*\*Sources\*\*
