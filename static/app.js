...
function antiSmash(text) {
    ...
    // FIX 2: Add space after punctuation: "Inc.,the" -> "Inc., the"
...
function renderAI(text) {
    try {
        ...
        // Parse JSON from web_search tool and render citations
        if(clean.includes('"title":') && clean.includes('"url":')) {
            try {
                const data = JSON.parse(clean);
                if(Array.isArray(data)) {
                    clean = '<div class="sources"><b>Sources:</b><br>' + 
                        data.map(d => `• <a href="${d.url}" target="_blank">${d.title}</a><br><small>${d.content}</small>`).join('<br>') + 
                        '</div>' + clean;
                }
            } catch(e) {}
        }
...
function addMessage(role, text, timestamp = null) {
    ...
    if(role === 'ai') {
        ...
        wrap.innerHTML = `<div class="message user-msg"><div class="msg-content">${antiSmash(text)}</div><div class="msg-meta">${time} ✓✓</div></div>`;
    }
