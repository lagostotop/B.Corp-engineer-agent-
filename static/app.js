...
// FIX 2: Get buttons after DOM loads
...
async function signIn() { 
    ...
    document.getElementById('loginBtn').disabled = true; // FIX 3: use getElementById
...
// FIX 4: Fixed regex that was breaking
function antiSmash(text) {
    ...
    text = text.replace(/([a-z])\.([A-Z])/g, '$1. $2');
    text = text.replace(/([a-z]):([A-Z])/g, '$1: $2');
    text = text.replace(/\s{2,}/g, '); // FIXED: was missing closing '
    return text.trim();
}
