...
let userId = "00000000-0000-0000-0000-000001"; // FIXED UUID - 36 CHARS
let isSending = false;
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('closeSidebarBtn').addEventListener('click', toggleSidebar); // CANCEL BUTTON
    ...
    // TAP OUTSIDE TO CLOSE SIDEBAR ON MOBILE
...
async function newChat() {
    ...
    document.getElementById('bcorpFace').style.display = 'block'; // SHOW INTRO
    if(window.innerWidth < 768) toggleSidebar(); // AUTO CLOSE AFTER NEW CHAT
    loadSidebar();
}

async function loadChat(chatId) {
    currentChatId = chatId;
    document.getElementById('bcorpFace').style.display = 'none'; // HIDE INTRO
    const res = await fetch(`/api/chat/${chatId}`);
    if(!res.ok) return;
    const messages = await res.json();
    document.getElementById('chatBox').innerHTML = '';
    messages.forEach(m => addMessage(m.role, m.content, m.created_at));
    if(window.innerWidth < 768) toggleSidebar(); // AUTO CLOSE AFTER OPENING CHAT
    loadSidebar();
}
...
function addMessage(role, text, timestamp = null) {
    document.getElementById('bcorpFace').style.display = 'none'; // HIDE INTRO ON FIRST MESSAGE
