...
// UPGRADE: GOD MODE ANTI-SMASH
function antiSmash(text) {
    if(!text) return '';
    // 1. Remove tool leaks
    text = text.replace(/web_search\s*\{[\s\S]*?\}/g, '');
    text = text.replace(/save_to_memory\s*\{[\s\S]*?\}/g, '');
    // 2. Fix smashed citations [1]"Title
    text = text.replace(/\[(\d+)\]"([^"]+)"/g, '[$1] "$2"');
    // 3. Fix camelCase and numbers
...
function renderAI(text) {
    try {
        ...
        if(clean.includes("Here's the plan:")) {
            clean = clean.replace("Here's the plan:", '<div class="agent-banner">🧠 Agent Mode</div>### Here\'s the plan:');
        }

        // UPGRADE: FIX SOURCES TO RENDER AS CARDS
        clean = clean.replace(/\*\*Sources\*\*\s*\n([\s\S]*)/g, (match, links) => {
            const linkMatches = [...links.matchAll(/\[(\d+)\] \[(.*?)\]\((.*?)\)/g)];
            if(linkMatches.length === 0) return match;
            let html = '<div class="sources"><b>Sources</b>';
            linkMatches.forEach(([_, num, title, url]) => {
                html += `<div class="source-card"><a href="${url}" target="_blank" rel="noopener">[${num}] ${title}</a></div>`;
            });
            html += '</div>';
            return html;
        });
        ...
        return `<div style="white-space:pre-wrap">${antiSmash(text)}</div>`;
    }
}
...
function addMessage(role, text, timestamp = null) {
    ...
    if(role === 'ai') {
        wrap.innerHTML = `<div class="message-avatar"><img src="/static/brain3d.png" alt="Brain"></div><div class="message ai-msg"><button class="copy-btn" onclick="copyText(this)">Copy</button><div class="msg-content">${renderAI(text)}</div><div class="msg-meta">${time}</div></div>`;
    } else {
...
async function sendMessage(questionOverride = null, isRegen = false) {
    ...
    try {
        ...
        aiMsgDiv.classList.add('typing-dots');
            ...
            for(const line of lines) {
                if(line.startsWith('data: ')) {
                    ...
                    if(data.startsWith('[CHAT_ID]')) {
                        ...
                        fullText += data;
                        aiMsgDiv.classList.remove('typing-dots');
        ...
        if(aiMsgDiv) {
            aiMsgDiv.classList.remove('typing-dots');
            aiMsgDiv.querySelector('.msg-content').innerHTML = `**Brain 3.0 Error**<br>${err.message}<br><small>Try regenerating or check server logs.</small>`;
        }
    } finally {
