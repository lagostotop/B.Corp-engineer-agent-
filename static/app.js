...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('fileInput').addEventListener('change', (e)=>{
        ...
        if(selectedFile) document.getElementById('uploadBtn').innerText = '📎1'; // Show file attached
    });
...
async function signUp() {
    ...
    if(error) document.getElementById('authError').innerText=error.message;
        ...
        document.getElementById('authError').innerText='Check email to confirm';
    }
}
...
// FIXED: Regex was breaking the build
...
- **See**: Upload images and I'll analyze them
...
async function sendMessage(questionOverride = null, isRegen = false) {
    ...
    try {
        ...
        while(true) {
            ...
            for(const line of lines) {
                if(line.startsWith('data: ')) {
                    ...
                    else if(data) { // FIXED: check data exists
                        fullText += data + ' ';
        ...
        document.getElementById('uploadBtn').innerText = '📎'; // Reset upload button
    }
  }
