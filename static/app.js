...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('sidebarOverlay').addEventListener('click', toggleSidebar); // FIX 1: CLOSE ON OVERLAY CLICK
...
function antiSmash(text) {
    if(!text) return '';
    text = text.replace(/web_search\s*\{.*?\}/g, '');
    text = text.replace(/save_to_memory\s*\{.*?\}/g, '');
...
function renderAI(text) {
    try {
        let clean = antiSmash(text); // ONLY CLEAN ONCE HERE
            ...
            for(const line of lines) {
                if(line.startsWith('data: ')) {
                    ...
                    if(data.startsWith('[CHAT_ID]')) {
                        ...
                        fullText += data; // FIX 2: ACCUMULATE RAW FIRST
                        aiMsgDiv.classList.remove('typing-dots');
                        aiMsgDiv.querySelector('.msg-content').innerHTML = renderAI(fullText); // RENDER FULL EACH TIME
                        document.getElementById('chatBox').scrollTop = document.getElementById('chatBox').scrollHeight;
                    }
                }
            }
