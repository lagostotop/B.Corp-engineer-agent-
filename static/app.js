...
async function newChat() {
    ...
    showIntroduction(); // SHOW INTRO ON NEW CHAT
...
async function loadChat(chatId) {
    ...
    document.getElementById('chatBox').innerHTML = '';
    if(messages.length === 0) showIntroduction(); // SHOW INTRO IF EMPTY
    else messages.forEach(m => addMessage(m.role, m.content, m.created_at));
    if(window.innerWidth < 768) toggleSidebar();
    loadSidebar();
}
...
// SHOW INTRODUCTION MESSAGE LIKE SCREENSHOT
