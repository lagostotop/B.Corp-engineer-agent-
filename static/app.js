...
let userId = "00000-0000-0000-0000-000001"; // MUST MATCH BACKEND GUEST_ID
let isSending = false;
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    loadSidebar();
    showIntroduction(); // AUTO SHOW ON LOAD
});
...
async function newChat() {
    ...
    introShown = false; // Reset so intro shows in new chat too
    showIntroduction(); // Show intro in new chat
...
async function loadChat(chatId) {
    ...
    document.getElementById('chatBox').innerHTML = '';
    if(messages.length === 0) showIntroduction(); // Show intro if chat is empty
