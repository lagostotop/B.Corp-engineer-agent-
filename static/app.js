...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('fileInput').addEventListener('change', (e)=>{ selectedFile = e.target.files[0]; if(selectedFile) document.getElementById('uploadBtn').innerText = '📎1'; });
    ...
    document.addEventListener('click', (e) => { const sidebar = document.getElementById('sidebar'); const hamburger = document.getElementById('hamburgerBtn'); if(window.innerWidth < 768 && sidebar.classList.contains('show')) { if(!sidebar.contains(e.target) &&!hamburger.contains(e.target)) { toggleSidebar(); } } });
    loadSidebar();
    showIntroduction();
});
...
async function sendMessage(questionOverride = null, isRegen = false) {
    ...
    try {
            ...
            for(const line of lines) {
                if(line.startsWith('data: ')) {
                    ...
                    if(data.startsWith('[CHAT_ID]')) {
                        ...
                        aiMsgDiv.classList.add('typing'); // Show loading
                    }
                    else if(data === '[DONE]') break;
                    else if(data) {
                        aiMsgDiv.classList.remove('typing'); // Hide loading
                        fullText += data;
        ...
        if(aiMsgDiv) { aiMsgDiv.classList.remove('typing'); aiMsgDiv.querySelector('.msg-content').innerHTML = `**Error: Server Error**<br>${err.message}`; }
    } finally {
