...
fetch('/config?v=43').then(res => {
    ...
    console.log("CONFIG LOADED", config.supabase_url) // ADDED LOG
    supabaseClient = supabase.createClient(config.supabase_url, config.supabase_key)
    checkAuth(); // This was running too early before
}).catch(err => {
...
async function checkAuth() { 
    ...
    const { data: { session }, error } = await supabaseClient.auth.getSession(); // CHANGED to await
    ...
    if(session) { 
        ...
        console.log("No session found"); // ADDED LOG
    }
}
...
// FIXED: Added loading state + better error
async function signIn() { 
    const email=document.getElementById('email').value; 
    const password=document.getElementById('password').value; 
    document.getElementById('authError').innerText = "Logging in..."; // ADDED
    ...
    if(error) {
        ...
        document.getElementById('authError').innerText = ""; // ADDED
        userId = data.user.id; 
