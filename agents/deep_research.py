def deep_research(topic: str, supabase, uid: str) -> str:
    """
    Phase 1 Deep Research v2: Plan -> Multi-search[Tavily+Docs] -> Extract -> Synthesize -> Cite
    """
    import time
    from tools.tavily import search_web # Use Tavily now
...
Return ONLY a JSON object: {{"questions": ["q1", "q2"]}}"""
    ...
    try:
        ...
        questions = plan_data.get("questions", [topic])[:4]
    ...
    for i, q in enumerate(questions):
        ...
        # 2a. Web Search with Tavily
        web_data = search_web(q)
        # Extract URLs for citations
        urls = re.findall(r'Source: (https?://[^\s]+)', web_data)
...
Be concise and factual. Include citation numbers [1][2] that map to sources. Topic: {topic}
...
Findings: {json.dumps(all_findings, indent=2)}"""
    ...
    sources_list = "\n".join([f"[{i+1}] {url}" for i, url in enumerate(list(dict.fromkeys(all_sources)))]) # dedupe
...
Cite sources inline as [1], [2] etc. At the end add a "Sources" section with the URLs.
...
{sources_list}"""
    ...
    final_res = client.chat.completions.create(
        model=REASONING_MODEL, # Use GPT-OSS 120B for synthesis
