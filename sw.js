const CACHE_NAME = 'brain3-cache-v4'; // bump version to force update
const STATIC_CACHE = [
  '/',
  '/static/manifest.json?v=44',
  '/static/app.js?v=44',
  '/static/icon-180.png',
  '/static/icon-192.png',
  '/static/icon-512.png',
...
self.addEventListener('fetch', event => {
  ...
  // 1. NEVER cache POST, /api/chat, /api/config, or supabase/CDN
  if (event.request.method !== 'GET' || 
      url.pathname.includes('/api/chat') || 
      url.pathname.includes('/api/config') ||
      url.pathname.includes('/api/chats') ||
      url.pathname.includes('/api/chat/') ||
