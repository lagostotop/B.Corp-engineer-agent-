import os,json,re,base64,uuid,logging,time,mimetypes
...
class AgentEngine:
    ...
    def run(self,goal,messages,file_meta,chat_id):
        ...
        agent_messages=[
            {
                "role":"system",
                "content":"You are Brain 3.0 by B.CORP. Use tools when necessary. Never reveal private chain-of-thought."
            }
        ]
        ...
        for _ in range(self.max_steps):
            ...
            for call in tool_calls:
                ...
                result_text=self._execute_tool(
                    call.function.name,args,file_meta,chat_id
                )
...
class ModelRouter:
    ...
    def _select_model(self,text,has_image):
        ...
        if len(x)<50 and any(
            word in x.split()
            for word in ("hi","hello","thanks","ok","yes","no")
        ):
            return self.FAST_MODEL

        if any(
            keyword in x
            for keyword in ("code","function","debug","algorithm","program","python","javascript")
        ):
    ...
    def route(
        ...
        has_image=bool(
            file_meta and
            file_meta.get("type") in IMAGE_EXTENSIONS
        )

        agent_result={"answer":""}

        if force_agent:
            agent_result=AgentEngine(
                self.supabase,uid
            ).run(
                last_user_msg,
                messages,
